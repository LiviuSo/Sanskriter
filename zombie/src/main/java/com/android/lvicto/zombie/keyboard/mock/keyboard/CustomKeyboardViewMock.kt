...
import com.android.lvicto.zombie.keyboard.ims.data.SuggestionsAdapter
import com.android.lvicto.zombie.keyboard.ims.view.key.*
import kotlinx.android.synthetic.main.row_suggestions.view.*

abstract class CustomKeyboardViewMock(context: Context, attr: AttributeSet) : ConstraintLayout(context, attr) {
                ...
                is ToggleKeyView -> {
                    ...
                    when (this.mCode) {
                        ...
                        context.resources.getInteger(R.integer.key_code_shift) -> {
                            shiftKeyView = this
                            this.setOnTouchListener(KeyListenersMock.getKeyShiftTouchListener())
                        }
                        else -> {
                            // nothing
                        }
                ...
                is BaseKeyView -> {
                    when (this.mCode) {
                        context.resources.getInteger(R.integer.key_code_space) -> { // space
                            this.setOnTouchListener(KeyListenersMock.getKeySpaceTouchListener())
                            setSpaceLabel(this)
                        }
                        context.resources.getInteger(R.integer.key_code_ac) -> { // action
                            this.setOnTouchListener(KeyListenersMock.getKeyActionTouchListener())
                        }
                        context.resources.getInteger(R.integer.key_code_backspace) -> { // delete
                            this.setOnTouchListener(KeyListenersMock.getKeyDeleteTouchListener())
                        }
                        context.resources.getInteger(R.integer.key_code_toggle_sys) -> { // toggle digits
                            this.setOnTouchListener(KeyListenersMock.getKeyToggleSystemBar())
                        }
                        else -> { // settings
                            this.setOnTouchListener(KeyListenersMock.getKeySettingsTouchListener())
                        }
                    }
                }
            }
