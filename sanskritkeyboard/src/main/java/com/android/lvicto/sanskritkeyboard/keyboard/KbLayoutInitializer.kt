...
abstract class KbLayoutInitializer(val context: Context) {
    ...
    val suggViewModel = SuggestionViewModel()
    ...
    fun setInputConn(inputConnection: InputConnection) {
        ic = inputConnection

    }

    lateinit var touchListners: TouchListners

    protected abstract fun getInstance(): KbLayoutInitializer
    abstract fun getAllCaps(): Boolean


    class TouchListners(val context: Context, val keyboardInit: KbLayoutInitializer) {
        ...
        val ic: InputConnection by lazy {
            keyboardInit.ic
        }
        ...
        fun updateKeyboard(view: View) { // todo add animations to switching
            (keyboardInit as KeyboardSwitch).switchKeyboard()
            // change label
        ...
        fun getSpaceKeyTouchListener() = object : View.OnTouchListener {
                ...
                return when (motionEvent.action) {
                    ...
                    MotionEvent.ACTION_UP -> {
                        ...
                        if (!longTap.get()) { // no tap; space key key_normal functionality
                            ...
                            val isQwerty = keyboardInit.getInstance() is KbLayoutInitPhoneQwertyPortrait
                            if (keyboardInit.isCapsFirstLetter && isQwerty && isStopExclamationQuestion()) {
                                (keyboardInit.getInstance() as KbLayoutInitPhoneQwertyPortrait).forceAllCaps(true)
                            }
                        }
                        keyboardInit.justAddSuggestions = false
        ...
        val settingsOnTouchListener = View.OnTouchListener { _, motionEvent ->
        ...
        val deleteOnTouchListener = object : View.OnTouchListener {
            ...
            val runnable = Runnable {
                ...
                keyView.post {
                    keyboardInit.showDigits()
                }
                keyView.background = ContextCompat.getDrawable(context, R.drawable.key_pressed)
                // delete the first char or the selected text
                if (keyboardInit.selectionEnd > keyboardInit.selectionStart) {
                    ic.setComposingRegion(keyboardInit.selectionStart, keyboardInit.selectionEnd)
                    val newCursorPosition = if (keyboardInit.selectionStartOld > 1) {
                        keyboardInit.selectionStartOld - 1
                    } else {
                        0
                    }
                ...
                keyboardInit.toggleAllCapsFirstLetter()
                ...
                while (!flagKeyDown.get()) {
                    if (System.currentTimeMillis() - actionTime > DELAY_AUTOREPEAT) {
                        ...
                        ic.deleteSurroundingText(1, 0)
                        keyboardInit.toggleAllCapsFirstLetter()
                        if (keyboardInit.mTypedText.isNotEmpty()) {
                            keyboardInit.mTypedText.delete(keyboardInit.mTypedText.length - 1, keyboardInit.mTypedText.length)
                        }
                        actionTime = System.currentTimeMillis()
                    }
                }
        ...
        val actionOnTouchListener = View.OnTouchListener { view, motionEvent ->
            when (motionEvent.action) {
                MotionEvent.ACTION_DOWN -> {
                    vibrateOnTap()
                    resetLongPressedKeys()
                    ...
                    val ei = keyboardInit.currentInputEditorInfo
                    if (ei.actionId != 0) {
                ...
                MotionEvent.ACTION_UP -> {
                    keyboardInit.updateSuggestions("")
                    keyboardInit.justAddSuggestions = true
                    view.background = ContextCompat.getDrawable(context, R.drawable.key_normal)
                    true
                }
                else -> {
        ...
        val suggestionOnTouchListener = View.OnTouchListener { view, motionEvent ->
            when (motionEvent.action) {
                MotionEvent.ACTION_DOWN -> {
                    val text = "${(view as Button).text}"
                    if (text.isNotEmpty()) {
                        if (!keyboardInit.justAddSuggestions) { // replace current word with suggestion
                            val lengthBefore = keyboardInit.getBeforeCursorInSurroundingWord().length
                            val lengthAfter = keyboardInit.getAfterCursorInSurroundingWord().length
                    ...
                    keyboardInit.mTypedText.delete(0, keyboardInit.mTypedText.length)
                    keyboardInit.showSuggestions()
                    true
                }
                MotionEvent.ACTION_UP -> {
                    keyboardInit.justAddSuggestions = true // suggestion was just tapped
                    true
                }
                else -> {
                    false
        ...
        val toggleDigitsTouchListener = View.OnTouchListener { view, motionEvent ->
            when (motionEvent.action) {
                MotionEvent.ACTION_DOWN -> {
                    vibrateOnTap()
                    resetLongPressedKeys()
        ...
        fun getSymbolKeyTouchListener(code: Int) = object : View.OnTouchListener {
                ...
                return when (motionEvent.action) {
                    MotionEvent.ACTION_DOWN -> {
                        ...
                        if (isSticky) {
                            ...
                            keyboardInit.showExtraKeys(code)
                        }

                        keyboardInit.showSuggestions(View.GONE, View.GONE, View.GONE)
                        keyboardInit.mTypedText.delete(0, keyboardInit.mTypedText.length)
        ...
        fun getCommonTouchListener(text: String = "", isExtra: Boolean = false) = object : View.OnTouchListener {
            ...
            private fun resetAndShowExtras(sticky: Boolean = false) {
                if (!sticky) {
                    // show extra keys
                    keyView.post { keyboardInit.showDigits() }
                }
                if (!isExtra) {
                    keyView.post { keyboardInit.showExtraKeys(output[0].toInt()) }
                }
                // toggle shift back (if not in permanent state)
                keyboardInit.toggleShiftBack()
        ...
        fun vibrateOnTap(isLongTap: Boolean = false) {
    ...
    protected open fun bindKeys(view: View) {
        touchListners = TouchListners(context, keyboardInit = this)
        (view button R.id.keySpace).apply {
            setOnTouchListener(touchListners.getSpaceKeyTouchListener())
        }
        view.findViewById<ImageButton>(R.id.keyDel).apply {
            setOnTouchListener(touchListners.deleteOnTouchListener)
        }
        (view imageButton R.id.keyAction).apply {
            actionButton = this
            setOnTouchListener(touchListners.actionOnTouchListener)
        }
    ...
    protected fun initExtraKeys(view: View) {
        toggleDigitsKey = (view imageButton R.id.keyToggleDigits).apply {
            setOnTouchListener(touchListners.toggleDigitsTouchListener)
        }
        ...
        extraKeys.forEach {
            it.setOnTouchListener(touchListners.getCommonTouchListener(isExtra = true))
        }
        view.post {
            showDigits()
        }
        ...
        if (getInstance() is KbLayoutInitPhoneQwertyPortrait) {
            ...
            if (!isAllCaps && isCapsFirstLetter) {
                ...
                if (isTypedTextEmpty()) {
                    ...
                    if (touchListners.isSpaceAfterStopExclamationQuestion()) {
                        kbQwertyInit.forceAllCaps(true)
                    } else {
